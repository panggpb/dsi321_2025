...
def data_processing(data: list[dict]) -> pd.DataFrame:
    """
    Process data, convert data type, minor cleaning and keeping necessary columns.
...
def load_to_lakefs(df: pd.DataFrame, lakefs_s3_path: str, storage_options: dict):
    ...
    Partitions the parquet file base on 'year', 'month', 'day' and 'hour'.
    ...
    storage_options: configured option for accessing lakeFS storage.
...
def main_flow():
    ...
    lakefs_s3_path = f"s3a://dsi321-record-{repo}/{branch}/{path}"
